...
export async function POST(
    ...
    try {
        ...
        if (!isAuthorized) {
            console.warn(`[API] Unauthorized approval attempt by user ${reviewerId}`);
            return NextResponse.json({ success: false, error: 'Unauthorized: You do not have permission to approve songs' }, { status: 403 });
        }

        console.log(`[API] Approving submission ${id} by ${reviewerName} (${reviewerId})`);
